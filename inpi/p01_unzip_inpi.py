...
def unzip():
    ...
    dirfile = {"fullpath": [], "annee_semaine": []}
    for dir in list_dir:
        for dirpath, dirs, files in os.walk(f"{PATH}{dir}/", topdown=True):
            if dirpath != f"{PATH}{dir}/":
                if "NEW" not in dirpath:
                    for item in files:
                        if item not in ["index.xml", "Volumeid"]:
                            flpath = dirpath + "/" + item
                            dirfile["fullpath"].append(flpath)
                            ppath = flpath.replace(PATH, "").split("/")
                            del ppath[2]
                            anse = "_".join(ppath)
                            dirfile["annee_semaine"].append(anse)
